...
此外，由于将两式相加，注意到$$ \xi_n $$和$$ \hat{\xi}_n $$是非负的，而$$ \epsilon $$是严格为正的，因此对于每个数据点$$ x_n,a_n $$或$$ \hat {a}_n $$至少一个为0，或都为0。所以$$ \epsilon + \xi_n + y_n − t_n = 0 $$和$$ \epsilon + \xi_n − y_n + t_n = 0 $$这两个限制是不兼容的。    
